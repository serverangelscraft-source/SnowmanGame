...
public class SchoolIntegrationActivity extends Activity {
    ...
    static class IntegrationView extends View {
        ...
        @Override protected void onDraw(Canvas c){super.onDraw(c);float w=getWidth(),h=getHeight(),bottom=h-safeBottom;LinearGradient g=new LinearGradient(0,0,0,h,Color.rgb(229,242,249),Color.rgb(250,244,227),Shader.TileMode.CLAMP);p.setShader(g);c.drawRect(0,0,w,h,p);p.setShader(null);RectF head=new RectF(dp(16),safeTop+dp(14),w-dp(16),safeTop+dp(142));p.setColor(Color.WHITE);c.drawRoundRect(head,dp(26),dp(26),p);center(c,"ДОДАТКОВА ПОДІЯ • "+grade+"-А",head.top+dp(25),7,Color.rgb(103,128,139));center(c,SchoolIntegrationContent.title(eventId),head.top+dp(62),15,Color.rgb(40,82,105));center(c,SchoolIntegrationContent.prompt(eventId),head.top+dp(94),7.5f,Color.rgb(90,119,132));center(c,"Не впливає на оцінки, монети чи прожитий день",head.top+dp(119),6.5f,Color.rgb(126,142,148));float top=head.bottom+dp(24),gap=dp(10),left=dp(20),cw=(w-dp(40)-gap*2)/3f,ch=Math.max(dp(118),Math.min(dp(170),(bottom-top-dp(112))));for(int i=0;i<3;i++){cards[i].set(left+i*(cw+gap),top,left+i*(cw+gap)+cw,top+ch);p.setColor(i==selected?Color.rgb(219,240,230):Color.WHITE);c.drawRoundRect(cards[i],dp(20),dp(20),p);drawIcon(c,cards[i].centerX(),cards[i].top+ch*.42f,i);centerAt(c,choices[Math.min(i,choices.length-1)],cards[i].centerX(),cards[i].bottom-dp(27),6.2f,i==selected?Color.rgb(52,126,96):Color.rgb(52,101,127));}
            if(selected>=0){done.set(dp(28),bottom-dp(72),w-dp(28),bottom-dp(14));p.setColor(Color.rgb(37,108,153));c.drawRoundRect(done,dp(20),dp(20),p);centerAt(c,"ЗБЕРЕГТИ СПОГАД",done.centerX(),done.centerY()+dp(4),8.4f,Color.WHITE);}else done.setEmpty();
        }
        ...
        void center(Canvas c,String s,float y,float size,int color){centerAt(c,s,getWidth()/2f,y,size,color);}void centerAt(Canvas c,String s,float x,float y,float size,int color){t.setTextAlign(Paint.Align.CENTER);t.setTextSize(tx(size));t.setColor(color);while(t.measureText(s)>Math.max(dp(70),getWidth()/3f-dp(18))&&t.getTextSize()>tx(4.8f))t.setTextSize(t.getTextSize()-dp(.2f));c.drawText(s,x,y,t);}
