...
public class SchoolIntegrationActivity extends Activity {
    ...
    @Override public void onBackPressed(){
        SchoolIntegrationSession.clear(getSharedPreferences("snowman_game",Context.MODE_PRIVATE));
        super.onBackPressed();
    }
    ...
    static class IntegrationView extends View {
        ...
        final float d,ts; final int grade,schoolDay,eventId; final String[] choices; float safeTop,safeBottom; int selected;
        IntegrationView(Activity a,int grade,int schoolDay){
            super(a);activity=a;this.grade=grade;this.schoolDay=schoolDay;eventId=SchoolIntegrationContent.eventFor(grade,schoolDay);choices=SchoolIntegrationContent.choices(eventId);prefs=a.getSharedPreferences("snowman_game",Context.MODE_PRIVATE);SchoolIntegrationSession.begin(prefs,grade,schoolDay);selected=SchoolIntegrationSession.savedChoice(prefs);d=getResources().getDisplayMetrics().density;ts=Math.min(getResources().getDisplayMetrics().scaledDensity,d*1.12f);t.setTypeface(Typeface.create(Typeface.DEFAULT,Typeface.BOLD));stroke.setStyle(Paint.Style.STROKE);setClickable(true);setFocusable(true);setOnApplyWindowInsetsListener(new OnApplyWindowInsetsListener(){@Override public WindowInsets onApplyWindowInsets(View v,WindowInsets i){safeTop=i.getSystemWindowInsetTop();safeBottom=i.getSystemWindowInsetBottom();invalidate();return i;}});requestApplyInsets();
        }
        ...
        @Override public boolean onTouchEvent(MotionEvent e){if(e.getAction()!=MotionEvent.ACTION_UP)return true;performClick();float x=e.getX(),y=e.getY();for(int i=0;i<3;i++)if(cards[i].contains(x,y)){selected=i;SchoolIntegrationSession.select(prefs,i);invalidate();return true;}if(selected>=0&&done.contains(x,y)){SchoolIntegrationSession.complete(prefs,grade,schoolDay,eventId,selected);activity.setResult(Activity.RESULT_OK);activity.finish();return true;}return true;}@Override public boolean performClick(){super.performClick();return true;}
    }
}
