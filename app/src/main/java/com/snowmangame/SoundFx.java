...
/** Tiny procedural game SFX engine. No external audio files are required. */
public final class SoundFx {
    ...
    public static boolean toggle(Context c){SharedPreferences p=c.getSharedPreferences("snowman_game",Context.MODE_PRIVATE);boolean on=!enabled(p);p.edit().putBoolean("sound_enabled",on).apply();if(on)play(c,UI,true);return on;}
    ...
    private static void play(Context c,int effect,boolean ignorePref){
        if(c==null)return;final Context app=c.getApplicationContext();final SharedPreferences p=app.getSharedPreferences("snowman_game",Context.MODE_PRIVATE);if(!ignorePref&&!enabled(p))return;
        long now=SystemClock.elapsedRealtime();int throttle=effect==CRUNCH?105:(effect==SLED?230:(effect==UI?65:35));
        synchronized(LAST){Long last=LAST.get(effect);if(last!=null&&now-last<throttle)return;LAST.put(effect,now);}
        final float volume=Math.max(.08f,Math.min(1f,p.getInt("sound_volume",68)/100f));final int seed=SEED.getAndIncrement();
        EXEC.execute(()->{if(!ignorePref&&!enabled(app))return;renderAndPlay(effect,volume,seed);});
    }

    private static void renderAndPlay(int effect,float volume,int seed){
        int ms=duration(effect),n=Math.max(64,SR*ms/1000);short[] pcm=new short[n];
        for(int i=0;i<n;i++){double t=i/(double)SR,u=i/(double)Math.max(1,n-1),v=sample(effect,t,u,i,seed);v=Math.max(-1,Math.min(1,v))*volume*.72;pcm[i]=(short)(v*32767);}
        AudioTrack track=null;try{
            ...
            if(track.getState()!=AudioTrack.STATE_INITIALIZED){track.release();return;}int written=track.write(pcm,0,pcm.length);if(written<=0)return;track.play();try{Thread.sleep(ms+35L);}catch(InterruptedException ignored){Thread.currentThread().interrupt();}
        }catch(Exception ignored){}finally{if(track!=null)try{track.stop();track.release();}catch(Exception ignored){}}
    }

    private static int duration(int e){switch(e){case CRUNCH:return 58;case SNOW_READY:return 220;case SNOW_SET:return 135;case ITEM:return 110;case ERROR:return 160;case COMPLETE:return 520;case MITTEN:return 210;case MEMORY:return 360;case PLAY:return 120;case HIT:return 115;case PHONE:return 240;case CAR_ARRIVE:return 430;case CAR_DOOR:return 170;case ENGINE:return 500;case TICKET:return 135;case TRAIN:return 560;case ARRIVAL:return 520;case MELT:return 360;case DRIP:return 180;case SCHOOL_BELL:return 720;case CORRECT:return 300;case WRONG:return 220;case CORE:return 620;case CLOTH:return 145;case PARCEL:return 230;case SLED:return 118;case TRAIN_DOOR:return 230;default:return 52;}}

    private static double sample(int e,double t,double u,int i,int seed){double a=attackRelease(u,.05,.20),noise=noise(seed,e,i)*a;switch(e){
        case UI:return Math.sin(2*Math.PI*760*t)*a*.38;
        case CRUNCH:return(noise*.72+Math.sin(2*Math.PI*118*t)*.20)*Math.pow(1-u,1.8);
    ...
    private static double attackRelease(double u,double attack,double release){double a=Math.min(1,u/Math.max(.001,attack)),r=Math.min(1,(1-u)/Math.max(.001,release));return Math.max(0,Math.min(a,r));}
    ...
    private static double noise(int seed,int effect,int i){int x=seed^(effect*0x9E3779B9)^(i*0x7F4A7C15);x^=x>>>16;x*=0x7feb352d;x^=x>>>15;x*=0x846ca68b;x^=x>>>16;return((x&0x7fffffff)/(double)0x3fffffff)-1.0;}
}
