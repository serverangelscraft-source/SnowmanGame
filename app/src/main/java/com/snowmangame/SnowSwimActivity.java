...
public class SnowSwimActivity extends Activity {
    ...
    static class SwimView extends View {
        ...
        int strokes=0;boolean expectLeft=true,done=false,counted=false;String feedback="Чергуй лівий і правий гребок.";
        SwimView(Context c){
            ...
            if(active){sessionDay=prefs.getLong("snow_swim_session_day",now);sessionWeekend=prefs.getBoolean("snow_swim_session_weekend",isWeekendDay(sessionDay));}
            else{sessionDay=now;sessionWeekend=isWeekendDay(now);prefs.edit().putBoolean("snow_swim_session_active",true).putLong("snow_swim_session_day",sessionDay).putBoolean("snow_swim_session_weekend",sessionWeekend).apply();}
        }
        ...
        void clearSession(){prefs.edit().remove("snow_swim_session_active").remove("snow_swim_session_day").remove("snow_swim_session_weekend").apply();}
        ...
        void countWeekend(){
            ...
            if(!can){
                if(!sessionWeekend)feedback="Тренування завершено. Ця сесія почалась не у вихідний.";
                else if(lastCompleted==sessionDay)feedback="Тренування завершено. Цей день уже врахований.";
                else if(lastCompleted>sessionDay)feedback="Тренування завершено. Стару сесію не можна зарахувати після новішого дня.";
                else feedback="Тренування завершено. Два вихідні цього року вже прожито.";
                clearSession();return;
            }
            weekendDone++;boolean yearDone=schoolDone>=5&&weekendDone>=2;
