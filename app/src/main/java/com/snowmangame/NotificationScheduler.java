...
/** Player-friendly local reminders for the school-life loop. */
public final class NotificationScheduler {
    ...
    public static void onSchoolOpened(Activity activity){
        ...
        p.edit().putLong("school_notify_last_open_day",today).apply();
    ...
    public static void onDayCompleted(Context context){
        scheduleNext(context);
    }

    public static void onSystemEvent(Context context){
    ...
    public static void scheduleNext(Context context){
        ...
        for(int offset=0;offset<16;offset++){
            ...
            int hour,minute;
            if(yearComplete){
                useful=day>completeDay;hour=11;minute=15;
            }else if(weekday){
                useful=school<5;hour=18;minute=20;
            }else{
                useful=weekend<2;hour=13;minute=10;
            }
            if(!useful)continue;
    ...
    private static long localDayNumber(){return localDayNumber(Calendar.getInstance());}
    private static long localDayNumber(Calendar c){Calendar copy=(Calendar)c.clone();copy.set(Calendar.HOUR_OF_DAY,0);copy.set(Calendar.MINUTE,0);copy.set(Calendar.SECOND,0);copy.set(Calendar.MILLISECOND,0);return copy.getTimeInMillis()/86400000L;}
}
