...
java=r'''package com.snowmangame;
...
public class SchoolWeekActivity extends Activity {
    ...
    static class SchoolWeekView extends View {
        ...
        void drawWeekendChoice(Canvas c){RectF r=card();cardBase(c,r);centerText(c,(calendarDow==Calendar.SATURDAY?"СУБОТА":"НЕДІЛЯ")+" • ВИХІДНИЙ "+(weekendDone+1)+"/2",r.top+dp(30),7,Color.rgb(109,132,140));centerText(c,"Школи сьогодні немає",r.top+dp(68),18,Color.rgb(43,105,139));centerText(c,"Обери справу. Вихідний теж треба реально прожити.",r.top+dp(98),7.4f,Color.rgb(99,125,136));String[] o=calendarDow==Calendar.SATURDAY?new String[]{"САНЧАТА","ПРОГУЛЯНКА","СНІЖКИ"}:new String[]{"СПОГАДИ","ЗІБРАТИ РЕЧІ","СНІЖИК"};float left=r.left+dp(18),right=r.right-dp(18),gap=dp(10),top=r.top+dp(132),cw=(right-left-gap*2)/3f;for(int i=0;i<3;i++){choices[i].set(left+i*(cw+gap),top,left+i*(cw+gap)+cw,top+dp(120));p.setColor(Color.rgb(238,248,251));c.drawRoundRect(choices[i],dp(18),dp(18),p);centerTextAt(c,o[i],choices[i].centerX(),choices[i].centerY()+dp(3),6.8f,Color.rgb(56,107,133));}drawHero(c,r.centerX()-dp(42),r.bottom-dp(40),dp(34));drawFriend(c,r.centerX()+dp(52),r.bottom-dp(40),dp(28));action.setEmpty();}
