...
# Five school-day scripts follow the ordinal of actual Mon-Fri dates inside the
# player's rolling seven-day snowman year. A player may start on any weekday.
...
# Header: day X/7 remains the snowman's seven-date year, but the phone weekday
# determines whether school is open.
rep(
    'c.drawText("День "+weekDay+"/7 • "+modeLabel(),r.left+dp(17),r.top+dp(52),text);',
    'c.drawText("День "+weekDay+"/7 • "+calendarDayName(),r.left+dp(17),r.top+dp(52),text);',
    'header real weekday',
)
rep(
    'c.drawText(schoolDay()?"5 навчальних днів • 2 вихідні • 1 день телефона = 1 день гри":"Сьогодні школа закрита. Рік усе одно продовжується.",r.left+dp(17),r.top+dp(78),text);',
    'c.drawText(schoolDay()?"НАВЧАННЯ • сьогодні "+mealDay+"-й навчальний день із 5":"ВИХІДНИЙ • школа сьогодні закрита",r.left+dp(17),r.top+dp(78),text);',
    'header mode copy',
)

# Dinner follows the nth actual school day, not the rolling day number.
rep('"ЕТАП 6/6 • ВЕЧЕРЯ "+weekDay+"/5"','"ЕТАП 6/6 • ВЕЧЕРЯ "+mealDay+"/5"','dinner counter')
rep('DISHES[weekDay-1]','DISHES[mealDay-1]','dinner title dish')
# The same expression appears again in drawDish, finishDay and done summary.
while 'DISHES[weekDay-1]' in s:
    s=s.replace('DISHES[weekDay-1]','DISHES[mealDay-1]',1)

# Weekends are actual Saturday/Sunday. Do not assume they are rolling days 6/7.
rep(
    'c.drawText("ВИХІДНИЙ "+(weekDay-5)+"/2",card.centerX(),card.top+dp(30),text);',
    'c.drawText(calendarDayName(),card.centerX(),card.top+dp(30),text);',
    'weekend title',
)
rep(
    'c.drawText(weekDay==6?"Суботній день без школи":"Тихий день перед новою зимою",card.centerX(),card.top+dp(67),text);',
    'c.drawText(calendarDow==Calendar.SATURDAY?"Суботній день без школи":"Недільний день без школи",card.centerX(),card.top+dp(67),text);',
    'weekend headline',
)
rep(
    'c.drawText(weekDay==6?"Обери одну справу. Більше навчальних завдань сьогодні немає.":"Обери, як провести останній день цього року.",card.centerX(),card.top+dp(96),text);',
    'c.drawText(calendarDow==Calendar.SATURDAY?"Обери одну справу на суботу.":"Обери одну справу на неділю.",card.centerX(),card.top+dp(96),text);',
    'weekend copy',
)
rep(
    'String[] o=weekDay==6?new String[]{"САНЧАТА","ПРОГУЛЯНКА","ВІДПОЧИНОК"}:new String[]{"СПОГАДИ","ЗІБРАТИ РЕЧІ","ПОБУТИ ЗІ СНІЖИКОМ"};',
    'String[] o=calendarDow==Calendar.SATURDAY?new String[]{"САНЧАТА","ПРОГУЛЯНКА","ВІДПОЧИНОК"}:new String[]{"СПОГАДИ","ЗІБРАТИ РЕЧІ","ПОБУТИ ЗІ СНІЖИКОМ"};',
    'weekend draw choices',
)
rep(
    'String[] o=weekDay==6?new String[]{"Санчата","Прогулянка","Відпочинок"}:new String[]{"Спогади","Зібрані речі","Час зі Сніжиком"};',
    'String[] o=calendarDow==Calendar.SATURDAY?new String[]{"Санчата","Прогулянка","Відпочинок"}:new String[]{"Спогади","Зібрані речі","Час зі Сніжиком"};',
    'weekend touch choices',
)
...
g=re.sub(r'versionName\s+"[^"]+"','versionName "18.3"',g)
